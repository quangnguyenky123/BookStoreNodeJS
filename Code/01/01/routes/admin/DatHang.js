...
router.get('/', function (req, res) {
	res.render('DatHang/DatHang.html');
});
router.post('/LoadData', async (req, res) => {
	...
	if (req.session.Quyen == 'Admin' || req.session.Quyen == 'Employee') {
		if (searchString != "") {
			...
			totalRow = (await DonDatHang.find({ MaSoDonDatHang: regex, IsBookStoreBuy: true })).length;
			if (totalRow > 0) {
				donDatHang = await DonDatHang.aggregate([
					{ $match: { MaSoDonDatHang: regex, IsBookStoreBuy: true } },
					{
						$lookup:
						{
					...
					},
					{
						...
						{
							from: "quantriviens",
							localField: "IdNguoiBan",
							foreignField: "_id",
							as: "NguoiBan"
						}
					}
				]).sort({ IdTrangThaiGiaoHang: 1 }).skip((page - 1) * pageSize).limit(pageSize);//lay du lieu ung voi so trang
			}
		}
		else {
			totalRow = (await DonDatHang.find({ IsBookStoreBuy: true })).length;//ko search
			if (totalRow > 0) {
				donDatHang = await DonDatHang.aggregate([
					{ $match: { IsBookStoreBuy: true } },
					{
						$lookup:
						{
							from: "trangthaigiaohangs",
	...
	if (req.session.Quyen == 'User') {
		if (searchString != "") {
			const regex = new RegExp(escapeRegex(searchString), 'gi');
			totalRow = (await DonDatHang.find({ MaSoDonDatHang: regex, IdKhachHang: mongoose.Types.ObjectId(req.session.UserId) })).length;
			if (totalRow > 0) {
				donDatHang = await DonDatHang.aggregate([
					{ $match: { MaSoDonDatHang: regex, IdKhachHang: mongoose.Types.ObjectId(req.session.UserId) } },
					{
						$lookup:
						{
							from: "trangthaigiaohangs",
			...
			totalRow = (await DonDatHang.find({ IdKhachHang: mongoose.Types.ObjectId(req.session.UserId) })).length;//ko search
			if (totalRow > 0) {
				donDatHang = await DonDatHang.aggregate([
					{ $match: { IdKhachHang: mongoose.Types.ObjectId(req.session.UserId) } },
					{
						$lookup:
						{
							from: "trangthaigiaohangs",
